...
const MX3Simulator: React.FC = () => {
  ...
  const [metrics, setMetrics] = useState<ServerMetric[]>([
    { id: 'app_srv', label: 'App Server (MX_CORE)', load: 15, status: 'HEALTHY' },
    { id: 'grid_p', label: 'Processing Grid (PRICING)', load: 8, status: 'HEALTHY' },
    { id: 'db_prd', label: 'Database (ORACLE_ENT)', load: 22, status: 'HEALTHY' }
  ]);
  ...
  const bookTrade = (asset: AssetClass, product: string, rate: number, notional: number) => {
    ...
    setTimeout(() => {
      ...
      logMessage('FpML', 'OUTBOUND', `<valutaionReport tradeId="${id}"><pv value="0"/></valuationReport>`);
    }, 1500);
  };
  ...
  const executeBatch = () => {
    ...
    setBatchProgress(0);
    setBatchLogs(['[BATCH] Starting EOD Cycle...', '[DB] Creating persistent snapshot...']);

    const steps = [
      { p: 25, m: '[PRICING] Running Monte Carlo VaR runs...' },
      { p: 50, m: '[GL] Generating General Ledger entries...' },
      { p: 75, m: '[OPS] Matching settlement instructions (SWIFT)...' },
      { p: 100, m: '[SUCCESS] Batch chain complete. Portfolio locked.' }
    ];
    ...
    <div className="max-w-full h-[calc(100vh-8rem)] flex flex-col bg-slate-950 rounded-[2.5rem] overflow-hidden border border-white/5 shadow-2xl font-sans text-slate-300">
      ...
      <header className="h-14 bg-slate-900 border-b border-white/10 flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-white italic">Industrial Live System</span>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-80 hidden lg:flex flex-col bg-slate-900/50 border-r border-white/5 overflow-hidden">
          <div className="p-4 border-b border-white/5 flex justify-between items-center">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Integration Bus (Live)</h3>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 font-mono text-[9px] custom-scrollbar">
            ...
            {messageBus.length === 0 && (
              <div className="h-full flex items-center justify-center text-slate-600 italic">Waiting for inbound data...</div>
            )}
          </div>
        </aside>
        ...
        <main className="flex-1 p-6 overflow-y-auto custom-scrollbar bg-slate-950">
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 space-y-4">
                  <h4 className="text-blue-400 font-black uppercase text-[10px] tracking-widest">Application Tier</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">Hosts MX.3 Core services, including the Pricing Engine and Workflow Orchestrator. Java/C++ performance layer.</p>
                  <div className="flex items-center gap-2 text-[10px] text-emerald-500 font-black"><i className="fas fa-check-circle"></i> CLUSTER_RUNNING</div>
                </div>
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 space-y-4">
                  <h4 className="text-emerald-400 font-black uppercase text-[10px] tracking-widest">Processing Grid</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">Distributes heavy calculations like VaR, Stress Tests, and Monte Carlo simulations across parallel nodes.</p>
                  <div className="flex items-center gap-2 text-[10px] text-emerald-500 font-black"><i className="fas fa-check-circle"></i> 124_NODES_ACTIVE</div>
                </div>
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 space-y-4">
                  <h4 className="text-amber-400 font-black uppercase text-[10px] tracking-widest">Database Tier</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">Oracle/RDBMS central repository for Trades, Static Data, and EOD state persistence.</p>
                  <div className="flex items-center gap-2 text-[10px] text-emerald-500 font-black"><i className="fas fa-check-circle"></i> CONNECTED: ORCL_P</div>
                </div>
              </div>

              <section className="bg-slate-900 rounded-[3rem] p-12 text-center border border-white/5 space-y-8 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none"><i className="fas fa-bolt text-[15rem]"></i></div>
                <div className="max-w-lg mx-auto space-y-4">
                  <h2 className="text-2xl font-black text-white uppercase tracking-tight">EOD Batch Cycle</h2>
                  <p className="text-slate-500 text-sm font-medium">Execute the End-of-Day transition to settle trades and publish reporting files.</p>
                </div>
                ...
                >
                  {isBatchRunning ? 'Executing...' : 'Run Daily Batch Chain'}
                </button>
          ...
          {activeDesk === 'FO' && (
            <div className="space-y-8 animate-in fade-in duration-500">
              <section className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5">
                <h3 className="text-lg font-black text-white mb-8 uppercase tracking-tight">Trading Console (Front Office)</h3>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-500 uppercase">Product Group</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-500 uppercase">Asset/Pair</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-500 uppercase">Trade Rate</label>
              ...
              <section className="bg-slate-900 border border-white/5 rounded-[2.5rem] overflow-hidden">
                <div className="px-6 py-4 bg-white/5 border-b border-white/5 flex justify-between items-center">
                  <h4 className="text-[10px] font-black text-white uppercase tracking-widest">Trade Repository Blotter</h4>
                ...
                <table className="w-full text-left text-[11px] font-mono">
                  <thead className="bg-white/5 text-slate-500">
                    <tr>
                      ...
                      <th className="px-6 py-4 uppercase text-right">Live P&L</th>
                  ...
                  <tbody className="text-slate-300">
                    {trades.map(t => (
                      <tr key={t.id} className="border-t border-white/5 hover:bg-white/5">
                        ...
                        <td className={`px-6 py-4 text-right font-black ${t.pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    ...
                    {trades.length === 0 && <tr><td colSpan={5} className="px-6 py-20 text-center text-slate-600 italic">No trades in active session repository.</td></tr>}
                  </tbody>
                </table>
              </section>
            </div>
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 space-y-6">
                  <h4 className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Portfolio Greeks</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-950 p-6 rounded-2xl border border-white/5">
                      <span className="text-[8px] font-black text-slate-500 uppercase mb-1 block">DV01</span>
                      <span className="text-xl font-black text-white font-mono">${(trades.length * 12.5).toFixed(2)}</span>
                    </div>
                    <div className="bg-slate-950 p-6 rounded-2xl border border-white/5">
                      <span className="text-[8px] font-black text-slate-500 uppercase mb-1 block">Vega</span>
                      <span className="text-xl font-black text-white font-mono">${(trades.length * 4.2).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
                <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 flex flex-col justify-center items-center text-center space-y-2">
                  <h4 className="text-[10px] font-black text-rose-400 uppercase tracking-widest">Risk Measure: VaR</h4>
                  <div className="text-5xl font-black text-white font-mono">${(trades.length * 2850).toLocaleString()}</div>
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Monte Carlo - 99.9% Confidence</p>
                </div>
              </div>
            </div>
          )}
          ...
          {activeDesk === 'BO' && (
            <div className="space-y-8 animate-in fade-in duration-500">
              <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-10 flex flex-col md:flex-row gap-8 items-center">
                <div className="flex-1 space-y-4">
                  <h3 className="text-xl font-black text-white uppercase tracking-tight">Ops & Settlement Logic</h3>
                  <p className="text-slate-500 text-sm leading-relaxed">Automation of SWIFT message generation (MT300/320) and Nostro reconciliation based on trade confirmation events.</p>
                  <div className="flex gap-4">
                    <div className="px-4 py-2 bg-slate-950 rounded-xl border border-white/5 flex flex-col">
                      <span className="text-[7px] font-black text-slate-500 uppercase">Matching Rate</span>
                      <span className="text-lg font-black text-emerald-400">99.2%</span>
                    </div>
                    <div className="px-4 py-2 bg-slate-950 rounded-xl border border-white/5 flex flex-col">
                      <span className="text-[7px] font-black text-slate-500 uppercase">Settlement Ratio</span>
                      <span className="text-lg font-black text-emerald-400">98.5%</span>
                    </div>
                  </div>
                </div>
                <div className="w-full md:w-64 space-y-3">
                   <button className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all">Regenerate Confirmations</button>
                   <button className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all">Manual Recon Fix</button>
                </div>
              </div>
            </div>
          )}
      ...
      <footer className="h-10 bg-slate-900 border-t border-white/10 px-8 flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-slate-600 shrink-0">
        <div className="flex gap-8">
          <span>HOST: HK_PRD_CLUSTER_01</span>
          <span>CURRENCY: USD</span>
          <span className="text-blue-500">ENGINE: V3.1.2_RELEASE</span>
        </div>
        <div className="flex gap-6 items-center">
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
            <span className="text-emerald-400">FRONT_TO_BACK_SYNCED</span>
          </div>
